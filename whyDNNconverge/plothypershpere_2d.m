...
start_u = plot(u(1), u(2), 'o', 'MarkerSize',8, 'MarkerFaceColor', start_u_color, 'MarkerEdgeColor', 'k', 'LineWidth', 2, 'DisplayName', '$x''_0$ Start'); % Plot v1 start marker
start_v = plot(v(1), v(2), 'o', 'MarkerSize',8, 'MarkerFaceColor', start_v_color, 'MarkerEdgeColor', 'k', 'LineWidth', 2, 'DisplayName', '$x_0$ Start'); % Plot v2 start marker
...
while ii<nter
    ...
    u = fu1;u=u/norm(u); u=softmax(u);

    
    % Encoding for v
    reduced_codeword2 = reduced_matrix * (v');
    fu2 = reduced_codeword2';
    v = fu2; v=v/norm(v); v=softmax(v);
    ...
    % Store current points as old points for the next iteration
        ...
        % Add arrows to represent trajectory
    quiver(old_u(1), old_u(2), u(1)-old_u(1), u(2)-old_u(2), 'Color', start_u_color, 'LineWidth', 1, 'MaxHeadSize', 0.2);
    quiver(old_v(1), old_v(2), v(1)-old_v(1), v(2)-old_v(2), 'Color', start_v_color, 'LineWidth', 1, 'MaxHeadSize', 0.2);
...
xlabel('${i}_{L}$', 'Interpreter', 'latex');
ylabel('${j}_{L}$', 'Interpreter', 'latex');
...
% Plot end points
end_u = plot(u(1), u(2), 's', 'MarkerSize',14, 'MarkerFaceColor', start_u_color,'MarkerEdgeColor', 'k', 'LineWidth', 2, 'DisplayName', '$x''_L$  End'); % Plot v1 end as black square
end_v = plot(v(1), v(2), 's', 'MarkerSize',14, 'MarkerFaceColor', start_v_color, 'MarkerEdgeColor', 'k','LineWidth', 2, 'DisplayName', '$x_L$ End'); % Plot v2 end as black diamond
...
% After plotting all your data and before 'hold off'

% Set aspect ratio to 'manual'
axis equal;
